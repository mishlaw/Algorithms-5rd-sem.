...
bool validate_input(int N, vector<Teacher>& teachers){
    ...
    if (total_required != N) {
        cout << "Ошибка: Общее количество требуемых пар (" << total_required
            << ") не совпадает с количеством доступных пар (" << N << ")\n";
        return false;
    }

    // Проверяем корректность занятых пар
    for (const Teacher& teacher : teachers) {
        for (auto busy_time : teacher.busy_intervals) {
            if (busy_time.first < 1 || busy_time.first > N || busy_time.second < 1 || busy_time.second > N) {
                cout << "Ошибка: Некорректный номер пары для преподавателя "
                    << teacher.name << "\n";
                return false;
            }
        }
